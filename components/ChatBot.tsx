...
export default function ChatBot() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end pointer-events-none">
      ...
      <div
        className={`w-96 max-w-[calc(100vw-2rem)] h-[520px] max-h-[calc(100vh-8rem)] mb-4 flex flex-col bg-white/95 backdrop-blur-xl border border-slate-200/85 shadow-[0_20px_50px_rgba(15,23,42,0.15)] rounded-2xl overflow-hidden pointer-events-auto transition-all duration-300 ease-in-out origin-bottom-right ${
          isOpen
      ...
      >
        ...
        {/* Header */}
        <div className="flex items-center justify-between p-4 bg-white/60 backdrop-blur-md border-b border-slate-200/60">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[11px] font-bold text-slate-500">Automations Expert</div>
            </div>
          </div>
          <button
            onClick={() => setIsOpen(false)}
            className="p-1.5 rounded-lg text-slate-400 hover:text-slate-900 hover:bg-slate-100 transition-all duration-200 cursor-pointer"
        ...
        <div className="flex-grow overflow-y-auto p-4 space-y-4 flex flex-col scrollbar-thin">
              ...
              <div
                className={`p-3.5 text-sm leading-relaxed whitespace-pre-line rounded-2xl ${
                  msg.sender === "user"
                    ...
                    : "bg-slate-50/90 border border-slate-200/70 text-slate-800 rounded-bl-none shadow-sm"
                }`}
              >
                {msg.text}
              </div>
          ...
          {isTyping && (
            <div className="flex items-end gap-2 max-w-[85%] self-start animate-pulse">
              ...
              <div className="bg-slate-50/90 border border-slate-200/70 rounded-2xl rounded-bl-none px-4 py-3.5 flex items-center gap-1.5 shadow-sm">
        ...
        {/* Input Bar */}
        <div className="p-4 bg-white/60 backdrop-blur-md border-t border-slate-200/60 flex items-center gap-2">
          <input
            type="text"
            value={inputValue}
            ...
            className="flex-grow bg-white border border-slate-200/80 rounded-xl px-4 py-2.5 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:border-brand focus:ring-4 focus:ring-brand/10 focus:shadow-[0_0_10px_rgba(14,179,186,0.08)] transition-all duration-300 disabled:opacity-50 placeholder:text-slate-400/80"
          />
          <button
            onClick={handleSend}
