...
export default function ContactForm() {
  ...
  return (
    <div className="relative group/form w-full max-w-2xl mx-auto">
      ...
      <div className="card-neon-border border border-brand-border rounded-[24px] p-6 sm:p-8">
        ...
        {status === "success" ? (
          <div className="text-center py-10">
            ...
            <p className="text-brand-muted mb-8 font-medium max-w-md mx-auto">
              {t.form_success_desc}
            </p>
            <button
              onClick={() => setStatus("idle")}
              className="px-6 py-2.5 rounded-xl border border-[var(--card-border-default)] hover:border-brand/40 text-sm font-semibold text-brand hover:text-brand-hover transition-colors duration-200 cursor-pointer"
            >
              {t.form_success_btn}
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                <label htmlFor="name" className="block text-xs font-bold uppercase tracking-wider text-brand-muted mb-2">
                  {t.form_name_label}
                </label>
                <input
                  type="text"
                  ...
                  required
                  className="w-full bg-[var(--form-input-bg)] border border-[var(--form-input-border)] hover:border-brand-hover rounded-xl px-4 py-3 text-[var(--foreground)] placeholder-brand-muted/60 focus:outline-none focus:border-brand focus:ring-4 focus:ring-brand/10 transition-all duration-300 placeholder:text-brand-muted/80"
                  placeholder={t.form_name_placeholder}
                />
              </div>
              <div>
                <label htmlFor="email" className="block text-xs font-bold uppercase tracking-wider text-brand-muted mb-2">
                  {t.form_email_label}
                </label>
                <input
                  type="email"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                <label htmlFor="company" className="block text-xs font-bold uppercase tracking-wider text-brand-muted mb-2">
              ...
              <div>
                <label htmlFor="service" className="block text-xs font-bold uppercase tracking-wider text-brand-muted mb-2">
                ...
                <select
                  ...
                  onChange={handleChange}
                  className="w-full bg-[var(--form-input-bg)] border border-[var(--form-input-border)] hover:border-brand-hover rounded-xl px-4 py-3 text-[var(--foreground)] focus:outline-none focus:bg-[var(--form-input-bg)] focus:border-brand focus:ring-4 focus:ring-brand/10 transition-all duration-300"
                >
            ...
            <div>
              <label htmlFor="message" className="block text-xs font-bold uppercase tracking-wider text-brand-muted mb-2">
                {t.form_msg_label}
              </label>
              <textarea
                id="message"
            ...
            <button
              ...
              className="btn-primary w-full inline-flex items-center justify-center rounded-xl bg-brand hover:bg-brand-hover px-8 py-4 text-base font-black tracking-wide uppercase transition-all duration-300 hover:scale-[1.01] disabled:opacity-50 cursor-pointer shadow-lg"
            >
              {status === "submitting" ? (
        ...
        <div className="mt-8 pt-6 border-t border-white/[0.05] text-center">
          ...
          <div className="flex items-center justify-center">
            <a
              ...
              className="group/mail flex items-center justify-center gap-2 w-full sm:w-auto px-6 py-2.5 border border-[var(--card-border-default)] hover:border-brand/40 hover:bg-brand/5 rounded-xl text-sm text-brand-muted hover:text-brand hover:shadow-md hover:shadow-brand/20 transition-all duration-300 font-semibold"
            >
              <svg
